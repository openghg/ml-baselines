...
def plot_monthly_means(monthly_means, shade_train_and_val_periods=True, site=None, obs_df=None):
    """
    ...
    ax.plot(monthly_means.index,
                monthly_means["true_monthly_mf"], label="True Baseline Monthly Mean", marker='o', color='darkgreen', markersize=3)
    ...
    ax.fill_between(monthly_means.index, 
                    ...
                    monthly_means["true_monthly_mf"] + monthly_means["true_monthly_std"],
                    color='darkgreen', alpha=0.3, label="True Baseline Monthly Std")
...
class BaselineLabelledObservations:
    ...
    def plot_monthly_means(self, shade_train_and_val_periods=True, plot_obs=True):
        if not hasattr(self, "monthly_means"):
            self.calculate_monthly_means()

        obs_df = self.labelled_df if plot_obs else None

        plot_monthly_means(self.monthly_means, shade_train_and_val_periods=shade_train_and_val_periods, site=self.site, obs_df=obs_df)